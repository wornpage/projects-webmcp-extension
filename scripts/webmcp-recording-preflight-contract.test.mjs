...
test('the CLI owns installed Edge, native viewport, exact keyboard order, diagnostics, and bounded cleanup', () => {
	...
	assert.match(harnessSource, /page\.goto\(spec\.productionUrl[\s\S]*?locator\('h1'[\s\S]*?assertFullscreenSettled\(page\)[\s\S]*?assertZeroOverflow\(page, 'landing-ready'\)[\s\S]*?const startedAt = performance\.now\(\)/u);
	...
	assert.match(harnessSource, /waitUntil\(startedAt, cueAt\('next-body-arrow-downs'\)\)[\s\S]*?bodyKeyScroll\(page, 'ArrowDown', spec\.keyboard\.nextBodyArrowDowns, 'next-prepared'\)[\s\S]*?waitUntil\(startedAt, cueAt\('next-to-work'\)\)[\s\S]*?pressExact\(page, 'Shift\+Tab', spec\.keyboard\.nextToWorkShiftTabs/u);
	assert.match(harnessSource, /waitUntil\(startedAt, cueAt\('final-body-arrow-downs'\)\)[\s\S]*?bodyKeyScroll\(page, 'ArrowDown', spec\.keyboard\.finalBodyArrowDowns, 'next-final'\)/u);
	...
	assert.equal((harnessSource.match(/#next-preparation-receipt/gu) ?? []).length, 2, 'Both restored-receipt checks own the canonical Next receipt id.');
...
test('runtime assertions substantively own catalogs, bounded actions, authority, trails, and diagnostics', () => {
	...
	assert.match(harnessSource, /const observePage = \(page\) => \{[\s\S]*?page\.on\('console'[\s\S]*?diagnostics\.consoleErrors\.push[\s\S]*?diagnostics\.cspFailures\.push[\s\S]*?page\.on\('pageerror'[\s\S]*?diagnostics\.pageErrors\.push/u);
