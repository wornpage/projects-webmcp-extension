...
test('static mode identifies its bounded routes without production fallbacks', () => {
	...
	assert.match(recordingScript, /Landing → Guide: six Tab presses[\s\S]*?Returned Guide → fast brief: two Shift\+Tab presses[\s\S]*?Work receipt → Review: seven Shift\+Tab presses[\s\S]*?Review receipt → Next: one Shift\+Tab press[\s\S]*?Reader API detected/u);
