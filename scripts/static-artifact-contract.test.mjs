...
test('static mode identifies its bounded routes without production fallbacks', () => {
	...
	assert.match(recordingScript, /Landing → Guide: press Tab on the page body to reclaim focus[\s\S]*?five additional Tab presses to reach \*\*Open the handoff workflow\*\*[\s\S]*?any earlier or later destination fails the take[\s\S]*?Guide → Priority: five Tab presses to reach \*\*Tools\*\*[\s\S]*?two Tab presses in the dialog reach \*\*Priority\*\*[\s\S]*?Priority → Guide: five Shift\+Tab presses[\s\S]*?Returned Guide → fast brief: five Shift\+Tab presses[\s\S]*?Fast brief → Work: press Tab once on the page body to reclaim page focus[\s\S]*?nine additional Tab presses to reach \*\*Work\*\*[\s\S]*?any earlier or later destination fails the take[\s\S]*?Work receipt → Review: thirteen Shift\+Tab presses to reach \*\*Tools\*\*[\s\S]*?three Tab presses in the dialog reach \*\*Review\*\*[\s\S]*?Review receipt → Next: three Shift\+Tab presses to reach \*\*Tools\*\*[\s\S]*?four Tab presses in the dialog reach \*\*Next\*\*[\s\S]*?Prepared Next receipt → Work: five Shift\+Tab presses[\s\S]*?Work Draft receipt → pending decision: fifteen Shift\+Tab presses to reach \*\*Pending 1\*\*[\s\S]*?two Tab presses in the dialog reach \*\*Review on Next\*\*[\s\S]*?upper view must first show the counted scope[\s\S]*?one PageDown pressed on the page body must then reveal brief `Use the WebMCP tools on Work, Review, and Next[\s\S]*?Do not save or change workspace data\.` and `Use fast-create brief` control, both fully visible[\s\S]*?Next: four ArrowDown presses on the page body must keep the prepared WebMCP receipt fully visible[\s\S]*?Pending 1[\s\S]*?eight ArrowDown presses[\s\S]*?Opening Guide frame:[\s\S]*?tool `WebMCP 1 tool`[\s\S]*?pill is ready only after Guide registration/u);
	...
	assert.match(recordingScript, /\| 00:25–00:30 \|[\s\S]*?let Codex invoke the registered Guide reader and follow the returned brief[\s\S]*?\| 01:00–01:18 \| Use three Shift\+Tab presses to open \*\*Tools\*\*, then four Tab presses to open Next[\s\S]*?hold the receipt, then send ArrowDown to the page body four times at 01:10[\s\S]*?\| 01:18–01:26 \| Use five Shift\+Tab presses to return to Work[\s\S]*?\| 01:36–01:50 \| Use fifteen Shift\+Tab presses and Enter on \*\*Pending 1\*\*, then two Tab presses and Enter on \*\*Review on Next\*\*[\s\S]*?hold the restored receipt, then send ArrowDown to the page body eight times at 01:43/u);
