...
test('static mode identifies its bounded routes without production fallbacks', () => {
	...
	assert.match(recordingScript, /Edge prompt: `Follow the brief on this page\.`[\s\S]*?Keep the safe default Guide brief; do not load \*\*Use fast-create brief\*\* for the Edge clip/u);
	assert.match(recordingScript, /In-app prompt: `Read this Work page, then create these as Draft work items: Confirm donation pickup window; Print shelf labels; Prepare bike rack checklist\. Do not start them\.`/u);
	assert.match(recordingScript, /Treat Edge chat as the trigger only[\s\S]*?Agent activity · WebMCP[\s\S]*?show_work_search[\s\S]*?set_review_scope[\s\S]*?prepare_next_action/u);
	assert.match(recordingScript, /## Edge primary clip — 00:00–02:05[\s\S]*?00:00–00:08[\s\S]*?01:35–02:05[\s\S]*?Do not activate Save or Discard during capture/u);
	assert.match(recordingScript, /## ChatGPT in-app WebMCP insert — 02:05–02:35[\s\S]*?submit the exact in-app prompt from Fixed setup[\s\S]*?get_current_work_view[\s\S]*?exactly one `create_work_drafts` call[\s\S]*?Created `3 · Draft`[\s\S]*?workspace `8 → 11`[\s\S]*?three visible Start controls/u);
	assert.doesNotMatch(recordingScript, /Before the timed take[\s\S]*?Use fast-create brief/u);
	assert.match(recordingScript, /## Post-capture cleanup and edit[\s\S]*?In Edge, discard the pending Next proposal[\s\S]*?in-app browser[\s\S]*?exact 8-item sample[\s\S]*?one hard cut at final time 02:05[\s\S]*?do not speed either clip/u);
	assert.match(recordingScript, /Abort the affected clip immediately[\s\S]*?Focus is lost after the Review-to-Next handoff[\s\S]*?CSP violation[\s\S]*?Do not hide it with the hybrid edit/u);
