...
test('static mode identifies its bounded routes without production fallbacks', () => {
	...
	assert.match(recordingScript, /Target final length: \*\*2:00\*\*\. Hard stop: \*\*2:15\*\*[\s\S]*?one continuous Microsoft Edge clip[\s\S]*?Do not speed up footage/u);
	...
	assert.match(recordingScript, /Landing → Guide: six Tab presses[\s\S]*?Returned Guide → fast brief: five Shift\+Tab presses[\s\S]*?press Tab once on the page body to reclaim page focus[\s\S]*?advance visibly with Tab until \*\*1 Work\*\* receives focus[\s\S]*?within five moves[\s\S]*?fail if focus reaches a later route first[\s\S]*?Work receipt → Review: seven Shift\+Tab presses[\s\S]*?Review receipt → Next: one Shift\+Tab press[\s\S]*?Prepared Next receipt → Work: four Shift\+Tab presses[\s\S]*?Reader API detected/u);
	...
	assert.match(recordingScript, /## One continuous Edge clip — 00:00–02:00[\s\S]*?00:00–00:07[\s\S]*?00:34–00:54[\s\S]*?01:12–01:36[\s\S]*?01:44–02:00/u);
