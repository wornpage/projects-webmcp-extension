...
test('static mode identifies its bounded routes without production fallbacks', () => {
	...
	assert.match(recordingScript, /preflight settles one rendered frame after every exact Tab or Shift\+Tab press[\s\S]*?requires the declared count and destination[\s\S]*?never searches for an alternate target/u);
	...
	assert.match(recordingScript, /Landing → Guide: press Tab on the page body to reclaim focus[\s\S]*?Open the handoff workflow[\s\S]*?within five additional moves[\s\S]*?Priority → Guide: seven Shift\+Tab presses[\s\S]*?Returned Guide → fast brief: three Shift\+Tab presses[\s\S]*?Fast brief → Work: press Tab once on the page body to reclaim page focus[\s\S]*?nine additional Tab presses to reach \*\*1 Work\*\*[\s\S]*?any earlier or later destination fails the take[\s\S]*?Work receipt → Review: seven Shift\+Tab presses[\s\S]*?Review receipt → Next: three Shift\+Tab presses[\s\S]*?Prepared Next receipt → Work: five Shift\+Tab presses[\s\S]*?Work Draft receipt → pending decision: ten Shift\+Tab presses[\s\S]*?upper view must first show the counted scope[\s\S]*?one PageDown pressed on the page body must then reveal the complete brief and fast-brief control[\s\S]*?Next: four ArrowDown presses on the page body[\s\S]*?Pending 1[\s\S]*?eight ArrowDown presses[\s\S]*?compact \*\*WebMCP\*\* pill shows `1 tool` and ready only after Guide registration/u);
