...
Those scoped successes now compose into one transient **Live WebMCP handoff** rail shared across Work, Review, and Next. It accumulates only verified action receipts—narrowed Work, verified Review, an unsaved Next proposal, staged Drafts, and the pending human decision—while continuously stating the agent authority boundary as `0 saved · 0 started`. The rail uses no storage, request, navigation, or workspace mutation path; Guide **Reset live sample** clears it together with the browser-local sample.
