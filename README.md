...
The gate runs Svelte diagnostics, focused WebMCP contracts, recording-preflight and static-artifact contracts, the content-addressed raster social-card smoke, and a production prerender. Current expected denominators are 127/127 public source paths, 134/134 WebMCP contracts, 6/6 recording-preflight contracts, and 8/8 static-artifact contracts. A static contract derives those values from the current manifest and test sources so this reviewer-facing summary cannot silently drift. Manual WebMCP checks are listed in [docs/submission/webmcp/reviewer-tests.md](docs/submission/webmcp/reviewer-tests.md).
